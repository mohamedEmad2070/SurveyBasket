    ...
    , ApplicationDbContext context) : IUserService
{
    ...
    public async Task<IEnumerable<UserResponse>> GetAllUserAsync(CancellationToken cancellationToken = default) =>
    ...
    public async Task<Result<UserResponse>> AddAsync(CreateUserRequest request,CancellationToken cancellationToken)
    {
        ...
        var allwedRoles = await _roleService.GetAllAsync(cancellationToken:cancellationToken);

        if (request.Roles.Except(allwedRoles.Select(r=>r.Name)).Any())
    ...
    public async Task<Result> UnlouckUserAsync(string userId)
    {
        if (await _userManager.FindByIdAsync(userId) is not { } user)
            return Result.Failure(UserErrors.UserNotFound);
