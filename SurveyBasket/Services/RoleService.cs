...
public class RoleService(RoleManager<ApplicationRole> roleManager, ApplicationDbContext context) : IRoleService
{
    ...
    public async Task<Result<RoleDetailResponse>> AddAsync(RoleRequest request)
    {
        ...
        var allwedPermissions = Permissions.GetAllPermissions();

        if (request.Permissions.Except(allwedPermissions).Any())
    ...
    {
        var roleIsExist = await _roleManager.Roles.AnyAsync(role => role.Id != id && role.Name == request.Name);
